...
public class QarApiBaseController : ControllerBase
{
    ...
    protected string CurrentLanguage => HttpContext.Request.Headers.TryGetValue("language", out var language)
        ? language.ToString()
        : QarCache.GetLanguageList(_memoryCache).FirstOrDefault(x => x.IsDefault == 1)?.Culture ?? string.Empty;
    ...
    public string T(string language, string localKey)
    {
        ...
        if (!languageList.Exists(x => x.Culture.Equals(language, StringComparison.OrdinalIgnoreCase)))
        {
            ...
            if (defaultLanguage != null) language = defaultLanguage.Culture;
        }
