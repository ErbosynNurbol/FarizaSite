...
public class AdminController : QarBaseController
{
    ...
    public IActionResult Login(string email, string password, string remember)
    {
        if (string.IsNullOrEmpty(email) || string.IsNullOrEmpty(email = email.Trim()))
            return MessageHelper.RedirectAjax(T("ls_Peye"), Status.Error, "", "email");
        if (!RegexHelper.IsEmail(email = email.Trim().ToLower()))
            return MessageHelper.RedirectAjax(T("ls_Peavea"), Status.Error, "", "email");
        if (string.IsNullOrEmpty(password) || string.IsNullOrEmpty(password = password.Trim()))
            return MessageHelper.RedirectAjax(T("ls_Peyp"), Status.Error, "", "password");
        using var connection = Utilities.GetOpenConnection();
        try
        {
            var admin = connection.GetList<Admin>("where qStatus = 0 and email = @email", new { email })
                .FirstOrDefault();
            if (admin == null)
                return MessageHelper.RedirectAjax(T("ls_Weop"), Status.Error, "", "");
            ...
            if (errorLog != null && errorLog.ErrorCount >= 5 && ts.TotalHours < 2)
                return MessageHelper.RedirectAjax(
                    "Құпия сөз терудің сәтсіз әрекеті көп болғандықтан жүйеге кіру 2 сағатқа бұғатталды ", Status.Error,
                    "", null);
            ...
            return MessageHelper.RedirectAjax(T("ls_Loginsuccessfully"), Status.Success,
                $"/{CurrentLanguage}/admin/profile", null);
        }
        catch (Exception exception)
        {
            Log.Error(exception, "Login");
            return MessageHelper.RedirectAjax("Сайт қателыгі, басқарушымен хабарласыңыз! ", Status.Error, "", null);
        }
    }
    ...
    public IActionResult ProfileInfo(Admin item)
    {
        ...
        if (string.IsNullOrWhiteSpace(item.Name))
            return MessageHelper.RedirectAjax(T("ls_Peyn"), Status.Error, "", "name");
        ...
        if (!string.IsNullOrWhiteSpace(item.Phone) && RegexHelper.IsPhoneNumber(item.Phone, out phoneNumber))
            return MessageHelper.RedirectAjax(T("ls_Petcpn"), Status.Error, "", "phone");
    ...
    public IActionResult ChangePassword(string oldPassword, string newPassword, string confirmPassword)
    {
        if (string.IsNullOrEmpty(oldPassword))
            return MessageHelper.RedirectAjax(T("ls_Peyp"), Status.Error, "", "oldPassword");

        if (string.IsNullOrEmpty(newPassword))
            return MessageHelper.RedirectAjax(T("ls_Peanp"), Status.Error, "", "newPassword");

        if (newPassword.Length < 6 || newPassword.Length > 20)
            return MessageHelper.RedirectAjax(
                T("ls_Pmcbmamc").Replace("{min}", "6").Replace("{max}", "20"),
                Status.Error, "", "newPassword");

        if (string.IsNullOrEmpty(confirmPassword))
            return MessageHelper.RedirectAjax(T("ls_Confirmnewpassword"), Status.Error, "", "confirmPassword");
        ...
        using (var connection = Utilities.GetOpenConnection())
        {
            ...
            if (!admin.Password.Equals(oldPassword))
                return MessageHelper.RedirectAjax(T("ls_Opii"), Status.Error, "", "oldPassword");
            admin.Password = Md5Helper.PasswordMd5Encrypt(newPassword);
            admin.UpdateTime = currentTime;
            if (connection.Update(admin) > 0)
                return MessageHelper.RedirectAjax(T("ls_Passwordchangedsuccessfully"), Status.Success,
                    $"/{CurrentLanguage}/admin/profile", "");
        }
    ...
    public IActionResult Role(Role item, string multiLanguageJson, string permissionJson)
    {
        ...
        if (string.IsNullOrEmpty(item.Name))
            return MessageHelper.RedirectAjax(T("ls_Thisfieldisrequired"), Status.Error, "", "name");
        ...
        catch (Exception ex)
        {
            ...
            return MessageHelper.RedirectAjax(T("ls_ErrordecodingJSONdata"), Status.Error, "", "multiLanguageJson");
        }
        ...
        {
            Log.Error(ex, ActionName);
            return MessageHelper.RedirectAjax(T("ls_ErrordecodingJSONdata"), Status.Error, "", "");
        }
        ...
        using (var _connection = Utilities.GetOpenConnection())
        {
            if (item.Id == 0)
            {
                ...
                if (res > 0)
                {
                    ...
                    return MessageHelper.RedirectAjax(T("ls_Addedsuccessfully"), Status.Success, $"/{CurrentLanguage}/{ControllerName.ToLower()}/{ActionName.ToLower()}/edit?id={res}", "");
                }
            }
            else
            {
                ...
                if (role == null)
                    return MessageHelper.RedirectAjax(T("ls_Isdeletedoridiswrong"), Status.Error, "", "");
                role.Name = item.Name;
                ...
                if (res > 0)
                {
                    ...
                    return MessageHelper.RedirectAjax(T("ls_Updatesuccessfully"), Status.Success, $"/{CurrentLanguage}/{ControllerName.ToLower()}/{ActionName.ToLower()}/edit?id={role.Id}", "");
                }
            }
    ...
    public IActionResult Person(Admin item, List<int> roleIdList)
    {
        ...
        if (roleIdList.Count <= 0)
            return MessageHelper.RedirectAjax(T("ls_Calo"), Status.Error, "", "roleIdList");

        if (string.IsNullOrEmpty(item.Name))
            return MessageHelper.RedirectAjax(T("ls_Tfir"), Status.Error, "", "name");

        if (string.IsNullOrEmpty(item.Email))
            return MessageHelper.RedirectAjax(T("ls_Tfir"), Status.Error, "", "email");
            ...
            if (item.Id == 0)
            {
                ...
                try
                {
                    if (connection.RecordCount<Admin>("where qStatus = 0 and email = @email",
                        ...
                        0)
                        return MessageHelper.RedirectAjax(T("ls_Eaiar"), Status.Error, "",
                            "email");
                    res = connection.Insert(new Admin
                    {
                        ...
                        HiddenColumnJson = "",
                        Phone = item.Phone,
                        LatynUrl = item.LatynUrl = GetDistinctLatynUrl(connection, nameof(Admin), "", item.Name, item.Id, ""),
                        ReLogin = 0,
                        SkinName = "light",
                        AddTime = currentTime,
                        UpdateTime = currentTime,
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (admin.IsSuper == 1 && admin.Id != GetAdminId())
                        return MessageHelper.RedirectAjax(T("ls_Accessdenied"), Status.Error, "", "");
