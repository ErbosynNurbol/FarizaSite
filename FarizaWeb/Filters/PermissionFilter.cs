            ...
            if (context.HttpContext.User.Identity.Role().Equals("Admin", StringComparison.OrdinalIgnoreCase))
            {
                    ...
                    loginTime < updateTime)
                {
                    switch (method)
                    {
                        ...
                        {
                            context.Result =
                                MessageHelper.RedirectAjax("Your session has timed out. Please login again.", Status.Error,
                                    "", null);
                            return;
                        }
                    }
                            ...
                            actionName.EndsWith("List", StringComparison.OrdinalIgnoreCase))
                        {
                            if (!canView)
                            {
                                context.Result = MessageHelper.RedirectAjax(
                                    QarCache.GetLanguageValue(memoryCache, "ls_Accessdenied", language), Status.Error, "",
                                    new
                                    {
