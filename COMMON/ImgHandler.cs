...
public static class ImgHandler
{
        ...
        int smallHeight = 100)
    {
        ...
        try
        {
            ...
            var sourceRect = new SKRectI(pointX, pointY, pointX + width, pointY + height);

            // Clamp to avoid out-of-bounds
            if (sourceRect.Left < 0) sourceRect.Left = 0;
            if (sourceRect.Top < 0) sourceRect.Top = 0;
            if (sourceRect.Right > thumbImg.Width) sourceRect.Right = thumbImg.Width;
            if (sourceRect.Bottom > thumbImg.Height) sourceRect.Bottom = thumbImg.Height;

            var cropWidth = sourceRect.Width;
            var cropHeight = sourceRect.Height;
            ...
            using (var canvas = new SKCanvas(croppedBitmap))
            {
                ...
                canvas.DrawBitmap(thumbImg, sourceRect, destRect);
            }

            // Only downscale for big/middle/small
            var bigTargetMaxW = smallWidth * 6;
            var bigTargetMaxH = smallHeight * 6;
            var middleTargetMaxW = smallWidth * 2;
            var middleTargetMaxH = smallHeight * 2;
            var smallTargetMaxW = smallWidth;
            var smallTargetMaxH = smallHeight;
            ...
            finalImgSmall = ResizeIfLarger(croppedBitmap, smallTargetMaxW, smallTargetMaxH);
            ...
            var finalPathBig = Path.Combine(webRoot, fArr[0] + "_big.webp");
            var finalPathMiddle = Path.Combine(webRoot, fArr[0] + "_middle.webp");
            var finalPathSmall = Path.Combine(webRoot, fArr[0] + "_small.webp");

            var saveFormat = SKEncodedImageFormat.Webp;
    ...
    #region Save Bitmap As +SaveBitmapAs(SKBitmap bmp, string path, SKEncodedImageFormat format)
    ...
    #region Resize If Larger +ResizeIfLarger(SKBitmap source, int maxW, int maxH)
    private static SKBitmap ResizeIfLarger(SKBitmap source, int maxW, int maxH)
    {
        ...
        float ratioW = (float)maxW / srcW;
        float ratioH = (float)maxH / srcH;
